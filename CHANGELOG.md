...
- **VULN-05 (Medium)** — Added `MinTenantIdLength` option (default `2`). `TenantFormatValidator` enforces the minimum in addition to the existing maximum, and `MultiTenantAuthOptionsValidator` rejects invalid combinations at startup.
...
- CI/CD: OIDC-based NuGet publishing via NuGet.org Trusted Publishing — no long-lived `NUGET_API_KEY` secret stored in GitHub.

### Changed

- `MultiTenantAuthOptionsValidator` now validates `MinTenantIdLength > 0` and `MinTenantIdLength <= MaxTenantIdLength`.
